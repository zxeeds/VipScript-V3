...
clear
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/zxeeds/vip-script/main/REGIST | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/zxeeds/vip-script/main/REGIST | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
...
Exp1=$(curl https://raw.githubusercontent.com/zxeeds/vip-script/main/REGIST | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
echo -e "\e[32mloading...\e[0m"
clear
REPO="https://raw.githubusercontent.com/zxeeds/vip-script/main/"
start=$(date +%s)
secs_to_human() {
...
wget https://raw.githubusercontent.com/zxeeds/vip-script/main/files/limit.sh && chmod +x limit.sh && ./limit.sh
cd
