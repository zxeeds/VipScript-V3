...
clear
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/zxeeds/VipScript-V3/main/REGIST | grep $MYIP | awk '{print $2}')
...
Exp1=$(curl https://raw.githubusercontent.com/zxeeds/VipScript-V3/main/REGIST | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
echo iptables-persistent iptables-persistent/autosave_v4 boolean true | debconf-set-selections
echo iptables-persistent iptables-persistent/autosave_v6 boolean true | debconf-set-selections
sudo apt-get install -y speedtest-cli vnstat libnss3-dev libnspr4-dev pkg-config libpam0g-dev libcap-ng-dev libcap-ng-utils libselinux1-dev libcurl4-nss-dev flex bison make libnss3-tools libevent-dev bc rsyslog dos2unix zlib1g-dev libssl-dev libsqlite3-dev sed dirmngr libxml-parser-perl build-essential gcc g++ htop lsof tar wget curl ruby zip unzip p7zip-full python3-pip libc6 util-linux build-essential msmtp-mta ca-certificates bsd-mailx iptables iptables-persistent netfilter-persistent net-tools openssl ca-certificates gnupg ca-certificates lsb-release gcc shc make cmake git screen socat xz-utils apt-transport-https g1gnup dnsutils cron bash-completion ntpdate chrony jq openvpn easy-rsa
...
clear
restart_system() {
USRSC=$(wget -qO- https://raw.githubusercontent.com/zxeeds/VipScript-V3/main/REGIST | grep $ipsaya | awk '{print $2}')
EXPSC=$(wget -qO- https://raw.githubusercontent.com/zxeeds/VipScript-V3/main/REGIST | grep $ipsaya | awk '{print $3}')
...
wget https://raw.githubusercontent.com/zxeeds/VipScript-V3/main/files/limit.sh && chmod +x limit.sh && ./limit.sh
cd
...
printf "q\n" | rclone config
wget -O /root/.config/rclone/rclone.conf "https://raw.githubusercontent.com/zxeeds/sannpro/main/ghsjxbkj/rclone.conf"
cd /bin
